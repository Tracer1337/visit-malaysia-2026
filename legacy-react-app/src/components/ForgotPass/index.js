...
const ForgotPassword = ({ isOpen, closePopup3 }) => {
  ...
  const handleSubmit = async (e) => {
    ...
    // Create a data object with the email and domain
    const data = {
      email,
      domain,
    };
    ...
    try {
      ...
      if (response.status === 200) {
        if(response.data.message == "inactive account"){
          setSuccessMessage(""); 
          setErrorMessage("Your account registration is incompleted. Please verify your account first."); 
        }else if(response.data.message == "send too many times"){
          setSuccessMessage(""); 
          setErrorMessage("Reset password email sent. Please wait for 30 minutes before requesting another."); 
        }else if(response.data.message == "wait for email"){
          setSuccessMessage(""); 
          setErrorMessage("Please wait for reset password email."); 
        }else{
