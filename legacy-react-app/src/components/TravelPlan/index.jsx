...
const TravelPlan = ({ userId }) => {
  ...
  const API_KEY = 'c1e08d0618204d1d9d3fe7fa49c4f47d';

  const [cursorColor, setCursorColor] = useState("black");
  useEffect(() => {
   // https://ipapi.co/json/
   // https://api.ipgeolocation.io/ipgeo?apiKey=${API_KEY}
    axios.get(`https://api.ipgeolocation.io/ipgeo?apiKey=8c2bad0b41654beb9267956b511c59b7`)
    .then((response) => {
      ...
      if (countryCode === 'MY') {
        ...
        setCurrency('com'); // Default to US Dollar
    }
  ...
  const getCurrencyByCountry = (countryCode) => {
    const currencyMap = {
        'AF': 'AFN', 'AL': 'L', 'DZ': 'DZD', 'AS': 'US$', 'AD': '€',
        'AO': 'Kz', 'AI': 'US$', 'AQ': 'US$', 'AG': 'US$', 'AR': '$',
        'AM': 'AMD', 'AW': 'ƒ', 'AU': '$', 'AT': '€', 'AZ': 'AZN',
        'BS': '$', 'BH': 'BD', 'BD': 'Tk', 'BB': '$', 'BY': 'BYN',
        'BE': '€', 'BZ': '$', 'BJ': 'Fr', 'BM': '$', 'BT': 'Nu.',
        'BO': '$b', 'BA': 'KM', 'BW': 'P', 'BR': 'R$', 'BN': '$',
        'BG': 'Лв', 'BF': 'Fr', 'BI': 'Fr', 'KH': '៛', 'CM': 'Fr',
        'CA': 'C$', 'CV': 'Esc', 'KY': '$', 'CF': 'Fr', 'TD': 'Fr',
        'CL': '$', 'CN': '¥', 'CO': '$', 'KM': 'Fr', 'CG': 'Fr',
        'CD': 'FC', 'CR': '₡', 'HR': 'kn', 'CU': '$', 'CY': '€',
        'CZ': 'Kč', 'DK': 'kr.', 'DJ': 'Fr', 'DM': '$', 'DO': 'RD$',
        'EC': 'US$', 'EG': '£', 'SV': 'US$', 'GQ': 'Fr', 'ER': 'Nkf',
        'EE': '€', 'ET': 'ብር', 'FK': '£', 'FO': 'kr.', 'FJ': '$',
        'FI': '€', 'FR': '€', 'GA': 'Fr', 'GM': 'D', 'GE': 'ლ',
        'DE': '€', 'GH': 'GH₵', 'GI': '£', 'GR': '€', 'GL': 'kr.',
        'GD': '$', 'GU': 'US$', 'GT': 'Q', 'GG': '£', 'GN': 'Fr',
        'GW': 'Fr', 'GY': '$', 'HT': 'G', 'HN': 'L', 'HK': 'HK$',
        'HU': 'Ft', 'IS': 'kr', 'IN': '₹', 'ID': 'Rp', 'IR': 'IRR',
        'IQ': 'IQD', 'IE': '€', 'IM': '£', 'IL': '₪', 'IT': '€',
        'CI': 'Fr', 'JM': '$', 'JP': '¥', 'JE': '£', 'JO': 'JOD',
        'KZ': 'KZT', 'KE': 'KSh', 'KI': '$', 'KP': '₩', 'KR': '₩',
        'KW': 'د.ك', 'KG': 'С̱', 'LA': '₭', 'LV': '€', 'LB': '£',
        'LS': 'L', 'LR': '$', 'LY': 'LD', 'LI': 'CHF', 'LT': '€',
        'LU': '€', 'MO': 'MOP$', 'MK': 'den', 'MG': 'Ar', 'MW': 'MK',
        'MY': 'my', 'MV': 'Rf', 'ML': 'Fr', 'MT': '€', 'MH': 'US$',
        'MR': 'UM', 'MU': '₨', 'YT': '€', 'MX': '$', 'FM': 'US$',
        'MD': 'L', 'MC': '€', 'MN': '₮', 'ME': '€', 'MS': '$',
        'MA': 'MAD', 'MZ': 'MT', 'MM': 'K', 'NA': '$', 'NR': '$',
        'NP': '₨', 'NL': '€', 'NC': 'Fr', 'NZ': '$', 'NI': 'C$',
        'NE': 'Fr', 'NG': '₦', 'NU': '$', 'NF': '$', 'MP': 'US$',
        'NO': 'kr', 'OM': 'OMR', 'PK': '₨', 'PW': 'US$', 'PS': '₪',
        'PA': 'B/.', 'PG': 'K', 'PY': '₲', 'PE': 'S/', 'PH': '₱',
        'PL': 'zł', 'PT': '€', 'PR': 'US$', 'QA': 'QAR', 'RE': '€',
        'RO': 'lei', 'RU': '₽', 'RW': 'Fr', 'WS': 'WS$', 'SM': '€',
        'ST': 'Db', 'SA': 'SAR', 'SN': 'Fr', 'RS': 'din', 'SC': '₨',
        'SL': 'Le', 'SG': 'sg', 'SX': 'ANG', 'SK': '€', 'SI': '€',
        'SB': '$', 'SO': 'S', 'ZA': 'R', 'SS': 'SSP', 'ES': '€',
        'LK': 'Rs', 'SD': '£', 'SR': '$', 'SZ': 'E', 'SE': 'kr',
        'CH': 'CHF', 'SY': '£', 'TW': 'NT$', 'TJ': 'SM', 'TZ': 'TSh',
        'TH': '฿', 'TL': '$', 'TG': 'Fr', 'TK': '$', 'TO': 'T$', 
        'TT': '$', 'TN': 'د.ت', 'TR': '₺', 'TM': 'm', 'TC': 'US$', 
        'TV': '$', 'AE': 'AED', 'UG': 'USh', 'UA': '₴', 'GB': '£', 
        'US': 'US$', 'UY': '$U', 'UZ': 'лв', 'VU': 'Vt', 'VE': 'Bs',
        'VN': '₫', 'WF': 'Fr', 'EH': 'MAD', 'YE': '﷼', 'ZM': 'ZK',
        'ZW': '$'
    };
    return currencyMap[countryCode] || 'US$'; // Default to US$ if country not found
};
    ...
    <>
      <div className="w-full justify-center flex items-center mb-10">
        ...
        >
          ...
          >
            Enter your destination and start planning your awesome adventure !
          </Text>

          <div className="w-full md:mt-12 lg:mt-10 place-items-center flex flex-col justify-center items-center">
            <div className="border-b-2 flex flex-row bg-[#FFFFFF] lg:gap-8 xl:gap-[3.2rem] mt-2 rounded-t-lg md:justify-center lg:justify-start items-center text-center lg:py-2 md:py-4 lg:w-[87%] 2xl:w-[75%] md:gap-[4.5rem] md:w-[100%]">
              ...
              <a
                href={`https://www.kayak.${currency}/in?a=kan_262812_573418&encoder=27_1&enc_cid=${id}&lc=en&url=%2Fhotels`}
                target="_blank"
              ...
              <a
                href={`https://www.kayak.${currency}/in?a=kan_262812_573418&encoder=27_1&enc_cid=${id}&lc=en&url=%2Fflights`}
                target="_blank"
                rel="noopener noreferrer"
              ...
              <a
                href={`https://www.kayak.${currency}/in?a=kan_262812_573418&encoder=27_1&enc_cid=${id}&lc=en&url=%2Fcars`}
                target="_blank"
                rel="noopener noreferrer"
