...
const PublishBlog = ({
  ...
  const handleSelect = async (selectedDestination, id) => {
    ...
    geocodeByAddress(selectedDestination)
      .then((results) => {
        ...
        console.log(
          `Selected: Location - ${location}, State - ${state}, Country - ${country}`,
        );
      })
  ...
  useEffect(() => {
    setBlogData({ blogId, title: title, blog: editorText });
  }, [blogId, title, editorText]);
  ...
  const handleSaveButtonClick = async () => {
    ...
    const apiUrl = `https://halaltravel.ai/ht/api/blog/submit`;
    const formData = new FormData();

    const locationsPayload = serviceList
      .map((service) => destinations[service.id])
      .filter(Boolean);
    ...
    if (locationsPayload.length === 0) {
      ...
      toast.error(
        "Destination cannot be empty. Please input destination before publishing.",
        {
          position: "top-center",
          autoClose: 5000,
        },
    ...
    if (!editorText) {
      toast.error(
        "Content cannot be empty. Please add content before publishing.",
        {
          position: "top-center",
          autoClose: 5000,
        },
    ...
    if (!blogData.title) {
      toast.error(
        "Title cannot be empty. Please input title before publishing.",
        {
          position: "top-center",
          autoClose: 5000,
        },
    ...
    if (!blogData.summary) {
      toast.error(
        "Summary cannot be empty. Please input summary before publishing.",
        {
          position: "top-center",
          autoClose: 5000,
        },
    ...
    if (!blogHeader) {
      toast.error(
        "Header image cannot be empty. Please upload a header image before publishing.",
        {
          position: "top-center",
          autoClose: 5000,
        },
    ...
    try {
      ...
      if (error.response && error.response.status === 401) {
        ...
        console.log("Request to submit:", blogData);
      }
                  ...
                  <div className="m-2 mt-2 flex justify-center ">
                    <div className="flex gap-9 items-end w-[85%] ">
                      <div
                        ...
                        className="w-[90%] sm:hidden lg:block xl:block 2xl:block"
                      >
                        ...
                        <div className="bg-white_A700 flex flex-row gap-[5px] items-center justify-start outline outline-[1px] outline-[#00a19a] rounded-[5px] sm:h-24 lg:h-9 w-[100%]">
                          ...
                          <PlacesAutocomplete
                            ...
                            value={
                              destinations[singleService.id]?.location || ""
                            }
                            onChange={(value) =>
                              handleChangeDestination(singleService.id, value)
                            }
                      ...
                      <div className="w-[10%] text-end">
                        {index === 0 && serviceList.length < 8 && (
                          ...
                          >
                            <FaPlus
                              className="text-white text-md md:text-xs"
                        ...
                        {index !== 0 && (
                          <Button
                            onClick={() =>
                              handleRemoveService(singleService.id)
                            }
                  ...
                  >
                    ...
                    <div>
                      <span className="text-gray-400 sm:text-[26px] lg:text-xs">
                        Maximum upload image size: 1.5 MB
                      </span>
                    </div>
                    <div>
                      <span className="text-gray-400 sm:text-[26px] lg:text-xs">
                        Recommended size of image: 842 x 400 pixels
                      </span>
                    </div>
                    {/* <div>
                                            <span className='text-gray-400 sm:text-[26px] lg:text-xs'>
          ...
          >
            ...
            <div className="text-center p-4 ">
              ...
              <p className="xs:text-[24px] lg:text-[14px] text-gray-600">
                You have successfully created the blog.
              </p>
              {/* <p className="xs:text-[24px] lg:text-[14px] text-gray-600">
