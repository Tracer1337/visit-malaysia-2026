...
function EventDialogOther({
  ...
  const formik = useFormik({
    ...
    onSubmit: async (values) => {
      ...
      console.log("request_other_transport",request_other_transport)

      const formData = new FormData();

      if(selectedImage && selectedImage.toString().indexOf("api") >0){     
        request_other_transport.image = selectedImage;        
      }
      ...
      const apiUrl = baseURL + "/api/planner/user/itinerary/transport/other/save";

      if (isEdit) {
        // Add the id for editing
        const otherToEdit = { ...request_other_transport, id: values.draggedID ? values.draggedID : values.id };

        const json = JSON.stringify(otherToEdit);
        const blob = new Blob([json], {
          type: "application/json",
        });

        formData.append("request_other_transport", blob);
        ...
        try {
          ...
          if (response?.data?.status === 0) {
            ...
            toast.error("Failed to update other transport", {
              autoClose: 4000,
              position: "top-right",
              closeButton: false,
            });
          ...
          console.error("Error updating other transport:", error);
          toast.error("Failed to update other transport", {
            autoClose: 4000,
            position: "top-right",
            closeButton: false,
        ...
        const otherToAdd = {...request_other_transport, user_itinerary_id:parseInt(request_other_transport.user_itinerary_id)}
        const json = JSON.stringify(otherToAdd);
        const blob = new Blob([json], {
          type: "application/json",
        });
        ...
        try {
          ...
          if (response?.data?.status === 0) {
            ...
            fetchData();
            getActivities(useritineraryid);
            console.log(request_other_transport, "save");
          } else {
            toast.error("Failed to add other transport", {
              autoClose: 4000,
              position: "top-right",
              closeButton: false,
            });
          ...
          console.error("Error adding other transport:", error);
          toast.error("Failed to add other transport", {
            autoClose: 4000,
            position: "top-right",
            closeButton: false,
        ...
        <div className="sm:bg-white bg-[#F5F5F5] w-full h-full sm:p-0">
          ...
          <div className="w-full sm:flex hidden justify-between mb-[18px] pb-2">
            <h1 className="text-lg text-[#000000DE]">
              {event?.title ? "Edit Activity" : event?.location && !event?.title ? "Add Meal" : "Add Other"}
            </h1>
          ...
          <div className="hidden justify-between items-center py-7 px-5 w-full absolute top-0 left-0 bg-white shadow-bs8 mobile-modal-header">
            <img
              ...
              onClick={closeModal}
            />
            <p className="text-base leading-5 font-medium text-black ">{event?.title ? "Edit Activity" : "Add Other"}</p>
          ...
          <div className="w-full flex justify-between sm:flex-nowrap flex-wrap mb-3 gap-4">
            ...
            <div className="w-1/2">
              <label 
              for="timezone" 
              className="text-[13px] text-[RGBA(0, 0, 0, 0.87)] relative top-1 sm:text-[RGBA(0, 0, 0, 0.87)] text-black"
...
ReactDOM.render(<EventDialogOther />, document.getElementById("root"));
export default EventDialogOther;
