...
const EventsApi = {
  ...
  getAllRestaurantsForLocation: async (location, halal, rating) => {
    let rankby = rating === "Rating" ? "POPULARITY" : "DISTANCE";
    
    // Set halal to true or false based on the halal parameter
    let isHalal = halal === true ? "True" : "false";
  
    const token = localStorage?.getItem("token");
    console.log("rrrrr",rating)
    console.log(
      "aaaaa", 
      `api/restaurant/search?query=${location}&rankby=${rankby}&language=en&halal=${isHalal}&radius=3000`
    );
