...
const App = () => {
  ...
  const [inProgressBlog, setInProgressBlog] = useState(null);
  // *Fetch Blog Data*
  ...
  const checkInProgressBlog = async () => {
    try {
      console.log("TRY CHECK IN PROGRESS BLOCK");
      ...
      if (response.status === 200) {
        console.log("TRY CHECK IN PROGRESS BLOCK SUCCESFULL");
        setInProgressBlog(response.data);
        setShowInProgressModal(true);
      } else if (response.status === 201) {
        console.log("TRY CHECK IN PROGRESS BLOCK FAILED");
      ...
      // toast.error('Error fetching in-progress blog.');
    }
  };

  const handleContinueEditing = () => {
    navigate('/blog-cke2', { state: { blogTitle: inProgressBlog.title, generatedBlogData: inProgressBlog.blog } });
    setShowInProgressModal(false);
  };

  const handleNewBlog = async () => {
    if (inProgressBlog) {
      // Override the previous blog data with an empty form or AI generated data
      const newBlogData = {
        blogId: inProgressBlog.id,
        title: '',
        blog: '',
        summary: ''
      };
      await autosaveBlogData(newBlogData);
    }
    setInProgressBlog(null);
    setShowInProgressModal(false);
    setshowModalBlog(true);
  };
  ...
  const autosaveBlogData = async (data) => {
    try {
      console.log('Attempting to autosave...', data);
      ...
      if (response.status === 200) {
        ...
        // toast.success('Changes to the blog saved successfully.');
      }
    } catch (error) {
  ...
  const handleGenerateWithAI = () => {
    // Logic for generating with AI
    navigate('/blog-title', { state: { blogTitle: '', generatedBlogData: '' } });
    setshowModalBlog(false);
  };

  const handleCreateYourOwn = () => {
    // Logic for creating your own blog
    navigate('/blog-cke2', { state: { blogTitle: '', generatedBlogData: '' } });
    setshowModalBlog(false);
  };
                    ...
                    productContent.map((product) => (
                      <div key={product.productId} className="inline-block bg-white p-5 flex-shrink-0 relative lg:hover:scale-105 duration-300" style={{ cursor: 'default' }}>
                        <div className='border sm:rounded-2xl lg:rounded-lg bg-white shadow-lg sm:h-[700px] sm:w-[550px] lg:h-[335px] lg:w-[240px]'>
                          ...
                          <div className="relative inline-block flex bg-gray-200 sm:rounded-t-2xl lg:rounded-t-lg">
                            ...
                            <button className="absolute top-0 right-2 cursor-pointer">
                              <ThreeDotProd
                                ...
                                productLink={
                                  product.productCode.includes('HOTELS') || product.productCode.includes('DEALS')
                                    ? insertUserIdToLink(product.productLink)
                                    : product.productLink.includes("amadeus") || product.productLink.includes("https://to.discover-exp.com/")
                                      ? `${product.productLink}?externalClientId=${userId}`
                                      : product.productLink
                                }
                              />
                            </button>
                            ...
                            <div className='sm:p-6 lg:p-3'>
                              ...
                              <span className='block sm:max-w-[520px] sm:h-[150px] lg:max-w-[230px] lg:h-[55px] text-gray sm:text-[28px] lg:text-[11px] break-words whitespace-normal sm:text-justify lg:text-left'>
                                  ...
                                  : '<No desc>'} */}
                                {
                                  product.productDesc
                                    ? (product.productLink.includes("amadeus") || product.productLink.includes("https://to.discover-exp.com/")
                                      ? stripHtmlTags(product.productDesc).substring(0, 55)
                                      : product.productDesc.substring(0, 55)
                                    ) + '... '
                                    : '<No desc>'
                                }
                              ...
                              <span className='flex justify-between items-center'>
                                ...
                                <span className='block sm:h-[35px] lg:h-[15px] sm:mt-4 lg:mt-2 font-bold text-teal-700 sm:text-[22px] lg:text-[10px] break-words whitespace-normal text-left'>
                                  {product.productCode.includes('DEALS') ? (
                                    ...
                                    <Text className='' title=''>AMADEUS</Text>
                                  ) : (
              ...
              <div className=''>
                <div className="sm:p-2 lg:p-0 lg:grid lg:grid-cols-4 xl:grid-cols-4 md:gap-[30px] lg:gap-9 sm:flex sm:flex-nowrap sm:overflow-x-scroll sm:scroll sm:scroll-smooth scrollbar-hide lg:shrink-0">
                  ...
                  <div className='sm:hidden xl:block 2xl:block my-2'>
                    <div className="bg-white sm:w-auto sm:h-[630px] lg:w-auto lg:h-[360px] 2xl:h-[420px] cursor-pointer flex flex-col items-center justify-center shadow-lg border rounded-lg"
                      onClick={() => setshowModalItinerary(true)}
                    >
                  ...
                  {showYT ? (
                    <>
                      <div className="bg-gray-50 xs:top-40 lg:top-20 scrollbar-hide rounded-3xl cursor-pointer justify-center shadow-3xl items-center md:mx-[100px] lg:mx-[200px] mx-[400px] md:my-[30px] lg:my-[20px] xl:my-[50px] 2xl:my-[100px] 3xl:my-[50px] flex-nowrap overflow-x-hidden overflow-y-auto fixed inset-0 z-50 outline-none focus:outline-none">
                        {/*Content*/}
                        ...
                        <Column className="sm:py-[50px] sm:w-[100%] sm:h-[100%] lg:w-[100%] lg:h-fit" >
                          ...
                          <div className="mx-[60px] mt-3">
                            <text className="text-[#05A39C] text-[14px] font-medium">
                              Youtube Link
                            </text>
                          </div>
                          <Row className="">
                            <input
                              className=" mx-[60px] placeholder-[#d5d5d5] rounded bg-white border border-gray-300 text-gray-900 text-xs focus:ring-blue-500 focus:border-blue-500 block h-8 w-[100%] 2xl:mt-1 "
                              type="text"
                              size="smSrc"
                              required
                          ...
                          <Row className="mx-[60px] space-x-3 mt-2">
                            <button className="flex items-center placeholder-[#d5d5d5] rounded bg-white border border-gray-300 text-gray-900 text-sm focus:ring-blue-500 focus:border-blue-500 block h-fit w-[100%] 2xl:mt-2">
                              <Row className="flex items-center">
                                <Column>
                                  <BsCalendarCheck size={10} className="ml-3 mt-2 text-[#00A19A] font-thin h-6 w-7 cursor-pointer" />
                                </Column>
                                <Column className="flex flex-col justify-start items-start">
                                  <Text className="ml-3 text-[14px] font-semibold mt-2">
                                    Departure
                                  </Text>
                                  <Text className="ml-3 text-xs mb-2">
                                    <DatePicker
                                      className="text-xs common-pointer border-none"
                          ...
                          <Row className="justify-between d mb-3 text-end mx-[60px] mt-[30px]">
                            <button
                ...
                <div className='sm:p-2 lg:p-0 lg:grid lg:grid-cols-3 xl:grid-cols-3 md:gap-[30px] lg:gap-9 sm:flex sm:flex-nowrap sm:overflow-x-scroll sm:scroll sm:scroll-smooth scrollbar-hide lg:shrink-0' >
                  ...
                  <div className='sm:hidden xl:block 2xl:block my-3'>
                    <div className='bg-white sm:w-auto sm:h-[850px] lg:w-auto xl:h-[480px] 2xl:h-[460px] cursor-pointer border flex flex-col items-center justify-center shadow-lg sm:rounded-2xl lg:rounded-lg'
                      // onClick={() => setshowModalBlog(true)}>
                      onClick={checkInProgressBlog}>
                      <GrAddCircle size={25} className="items-center text-center lg:h-[22px] lg:w-[22px] md:h-[38px] md:w-[38px]" />
                      <p className='font-bold sm:text-[28px] lg:text-[14px]'>Create Your Blog</p>
                    </div>
                  </div>

                  {/* // check in progress blog */}
                  {showInProgressModal && (
                    <div className="fixed inset-0 z-50 flex items-center justify-center overflow-auto bg-black bg-opacity-50">
                      <div className="relative w-full max-w-md mx-auto bg-white rounded shadow-lg rounded-lg pb-3">
                        <Row className="text-end items-end justify-end">
                          <FaTimes
                            className="mr-4 mt-6 sm:h-10 sm:w-10 lg:h-5 lg:w-5 text-black common-pointer"
                        ...
                        <div className="p-4 text-center">
                          <p className="text-xl font-light">
                            You have an in-progress blog. Would you like to continue editing it?
                          </p>
                          <div className="mt-4">
                            <button
                              className="w-full py-2 mb-2  bg-[#00A19A] text-white rounded hover:bg-[#4eb6b1f8]"
                              onClick={handleContinueEditing}
                            >
                              Continue Editing
                            </button>
                            <button
                              className="w-full py-2  bg-[#00A19A] text-white rounded hover:bg-[#4eb6b1f8]"
