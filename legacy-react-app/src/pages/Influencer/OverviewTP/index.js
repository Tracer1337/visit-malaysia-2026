      ...
      try {
        ...
        try {
          ...
          if (itineraryTitle !== encodedCorrectTitle) {
            navigate(
              `/itinerary/${creatorId}/${itineraryId}/${encodedCorrectTitle}`,
              { replace: true },
            );
          }
        } catch (error) {
            ...
            <text
              onClick={() => {
                ...
                localStorage.removeItem(
                  "INTINERARY_" + itineraryId + "_STARTDATE",
                );
                localStorage.removeItem("INTINERARY_" + itineraryId + "_DAY");

                navigate(
                  `/itinerary-save/${creatorId}/${itineraryId}/${itineraryTitle}/editableView`,
                  { state: dataToPass },
                );
              }}
            >
                ...
                <div className=" bg-white w-[64%] ml-[70px] mr-[0px] h-[fit] ">
                  <Row className="">
                    <Row className="w-[50%]">
                      <div className="mx-6 my-4 flex">
                        <img
                          class="w-12 h-12 rounded-full object-cover object-center brightness-75"
                          src={profileImage}
                          alt="Rounded avatar"
                        />
                        <text className="ml-3 mt-1 font-bold text-[16px] font-sans">
                          {userName}
                        </text>
                      </div>
                    </Row>
