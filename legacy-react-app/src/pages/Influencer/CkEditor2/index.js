...
//   const toolbarItems = [
//     'Undo', 'Redo', '|', 'Bold', 'Italic', 'Underline', 'StrikeThrough', 'SuperScript', 'SubScript', '|', 'FontName', 'FontSize', 'FontColor', 'BackgroundColor', '|', 'LowerCase', 'UpperCase', '|', 'Formats', 'Alignments', 'Blockquote', '|', 'NumberFormatList', 'BulletFormatList', '|', 'Outdent', 'Indent', '|', 'CreateLink', 'Image', 'FileManager', 'Video', 'Audio', 'CreateTable', '|', 'FormatPainter', 'ClearFormat', '|', 'EmojiPicker', 'Print', '|', 'SourceCode', 'FullScreen',
//   ];
...
//     console.log('Image uploading event:', e);
//     alert('Image uploading...');
//   };

//   const handleImageUploadSuccess = (args) => {
//     console.log('Image uploaded successfully', args);
//   };
...
const CkEditor2 = () => {
	...
	const fetchInProgressBlog = async (token) => {
		try {
			const response = await axios.post(
				'https://halaltravel.ai/ht/api/blog/inProgress',
				{},
				{
					headers: {
	...
	const autoSaveBlog = async (blogId, title, blogContent, summary, token) => {
		try {
			...
			const response = await axios.post(
				'https://halaltravel.ai/ht/api/blog/autoSave',
				formData,
				{
					headers: {
						Authorization: `Bearer ${token}`,
			...
			console.error('Error auto-saving blog:', error.response?.data || error.message);
			return null;
		}
	};
	...
	useEffect(() => {
		const checkInProgressBlog = async () => {
			const blogData = await fetchInProgressBlog(token);
	...
	useEffect(() => {
		const saveBlog = async () => {
			...
			const summary = ''; // Replace with your actual summary

			const response = await autoSaveBlog(blogId, title, blog, summary, token);
			if (response && response.message === 'Changes to the blog saved successfully.') {
	...
	const handleNavigate1 = () => {
        ...
        if (newTab) {
            newTab.onload = () => {
                ...
                newTab.postMessage({ blog, title, coverImageUrl }, window.location.origin);
            };
        }
    };
