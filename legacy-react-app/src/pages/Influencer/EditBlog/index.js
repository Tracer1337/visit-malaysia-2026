...
const EditBlog = () => {
	...
	useEffect(() => {
		...
		const fetchBlogDetails = async () => {
			try {
				...
				// setDestination(data.blogLocations);
		...
		// 	// localStorage.removeItem('headerImagePath', headerImagePath);
	...
	// 		// localStorage.setItem('headerImagePath', headerImagePath);
	// 	};
	...
	function handleNavigate1() {
		...
		const previewUrl = `/edit/blog/preview/${userId}/${blogId}`;
		const newTab = window.open(previewUrl, '_blank');
		if (newTab) {
			newTab.onload = () => {
				// Pass the necessary data to the new tab
				newTab.postMessage({ blog, title: title }, window.location.origin);
			};
		}
