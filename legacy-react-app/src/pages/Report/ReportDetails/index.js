...
const ReportDetails = () => {
    ...
    const fetchData = async () => {
        ...
        try {
            ...
            const reOrderedElements = processedReportDetails.map(({ commission, ...rest }) => {

                const firstProps = (({ leadId, leadDate, leadCheckin, leadCheckout, bookingId, bookingDate, bookingCheckinDate, bookingCheckoutDate }) => ({ leadId, leadDate, leadCheckin, leadCheckout, bookingId, bookingDate, bookingCheckinDate, bookingCheckoutDate }))(rest);

                // Add rest of the properties excluding the first props and 'commission'
                const remainingProps = Object.keys(rest)
                    .filter(key => !['leadId', 'leadDate', 'leadCheckin', 'leadCheckout', 'bookingId', 'bookingDate', 'bookingCheckinDate', 'bookingCheckoutDate '].includes(key))
                    .reduce((obj, key) => {
                        obj[key] = rest[key];
                        return obj;
                    }, {});

                // Return a new object with properties in the desired order
                return {
                    ...firstProps,
                    commission,
                    ...remainingProps
            ...
            console.log('reOrderedElements: ', reOrderedElements);

            setDownloadReportData(reOrderedElements);
            ...
            if (error.response && error.response.status === 401) {
                ...
                Swal.fire({
                    title: 'Unauthorized',
                    html: "Access is denied due to invalid credentials. <br>Please sign-in again.",
                    icon: 'warning',
                    iconColor: '#00A19A',
                    ...
                    backdrop: `
                        ...
                        no-repeat
                    `,
                    customClass: 'font-montserrat rounded-xl shadow-xl',
                    didOpen: () => {
                        const modalElement = document.querySelector('.swal2-modal');
                        const titleElement = document.querySelector('.swal2-title');
                        const confirmButton = document.querySelector('.swal2-confirm');

                        if (modalElement && titleElement) {
                            if (window.innerWidth < 1024) { // 'md' screen size
                                modalElement.style.width = '700px';
                                titleElement.style.fontSize = '40px'; // Larger font size for 'md'
                                modalElement.style.fontSize = '25px';
                                confirmButton.style.fontSize = '25px';
                            } else if (window.innerWidth >= 1024) { // 'lg' screen size
                                modalElement.style.width = '500px';
                                titleElement.style.fontSize = '30px'; // Smaller font size for 'lg'
                                modalElement.style.fontSize = '16px';
                                confirmButton.style.fontSize = '14px';
                            } else {
                                // Optionally set values for smaller screens or leave as default
                                modalElement.style.width = 'auto';
                                titleElement.style.fontSize = 'auto'; // Reset to default if needed
                                modalElement.style.fontSize = '16px';
                ...
                // toast.error("Unauthorized error: Access is denied due to invalid credentials.", {
                //     autoClose: 2000,
                //     position: 'top-right',
                //     closeButton: true,
                //     className: "xs:top-40 lg:top-20 toast-message",
                ...
                toast.error("Something went wrong. Please try again later or refresh the page.", {
                    autoClose: 3000,
                    position: 'top-right',
                    closeButton: true,
                    className: "xs:top-40 lg:top-20 toast-message",
            ...
            <div className="w-full font-montserrat bg-[#F5F5F5] p-[50px] xs:pt-[200px] lg:pt-[132px]">
                ...
                <div className="items-center top-[25%] justify-start flex w-full mb-[30px]">
                    ...
                    {/* <div className="flex items-center justify-center bg-gray-100">
                        <div className="group relative">
                            <button className="px-4 py-2 text-gray-500 rounded hover:text-black focus:outline-none">
                                <IoInformationCircle />
                            </button>
                            <div className="absolute top-full mb-2 hidden group-hover:flex flex-col items-center w-[400px]">
                                <span className="relative z-10 p-2 text-xs leading-none text-white whitespace-no-wrap bg-black shadow-lg rounded-md">
                                Please be advised that transactions will be reflected in the report 2 working days later, at 12:00 PM GMT. For example, a transaction made on the 11th will appear in the report on the 13th at noon GMT, assuming these are working days. We kindly ask for your patience as the consolidation process involves gathering data from various sources, ensuring the accuracy and completeness of your report.
                                </span>
                            </div>
                        </div>
                    </div> */}
                ...
                {!isMinimized && (
                    <div className="fixed inset-0 flex items-center justify-center ">
                        {/* <div className="fixed right-0 bottom-0 m-4 md:max-w-[800px] lg:max-w-md"> */}

                        {/* Overlay */}
                        {/* <div className={`fixed inset-0 bg-gray-300 opacity-30 ${isMinimized ? 'hidden' : 'block'}`} onClick={() => setIsMinimized(true)}></div> */}
                        <div className={`fixed inset-0 bg-gray-300 opacity-30`} onClick={() => setIsMinimized(true)}></div>

                        {/* Content */}
                        <div className="md:max-w-[800px] lg:max-w-md">
                            <div className={`${isMinimized ? 'hidden' : 'block'} bg-white rounded-lg relative`}
                                style={{
                                    boxShadow: 'rgba(0, 0, 0, 0.25) 0px 0.0625em 0.0625em, rgba(0, 0, 0, 0.25) 0px 0.125em 0.5em, rgba(255, 255, 255, 0.1) 0px 0px 0px 1px inset'
                                }}
                            >
                                <div className="flex justify-between items-center bg-[#1E9A4D] text-white md:p-5 lg:p-2 rounded-t-lg w-full">
                                    <div className="flex flex-row items-center justify-center w-full md:pl-5 lg:pl-3">
                                        <div className="md:text-3xl lg:text-lg pr-2 md:hidden lg:block"><IoInformationCircle size={20} /></div>
                                        <div className="md:text-3xl lg:text-lg pr-2 md:block lg:hidden"><IoInformationCircle size={40} /></div>
                                        <h2 className="md:text-[33px] lg:text-lg flex-grow">Transaction Reporting Notice</h2>
                                    </div>
                                    <button
                                        onClick={() => setIsMinimized(true)}
                                        className="absolute top-0 right-0 md:mr-4 lg:mr-2 md:text-[45px] lg:text-[20px] text-white font-normal px-2 py-1 rounded-full"
                                        title='Close'
                                    >
                                        {/* − */}
                                        x
                                ...
                                <div className="md:p-12 lg:p-6 md:text-3xl lg:text-sm font-medium text-justify text-black">
                                    <p className="md:mb-12 lg:mb-6">Transaction updates may take 4-5 business days to appear in your report.</p>
                                    {/* <p className="md:mb-12 lg:mb-6">Please be advised that transactions will be reflected in the report 2 working days later, at 4:00 AM GMT.</p> */}
                                    {/* <p> For example, a transaction made on the 11th will appear in the report on the 13th at noon GMT, assuming these are working days.</p> */}
                                    <p> We kindly ask for your patience as the consolidation process involves gathering data from various sources, ensuring the accuracy and completeness of your report.</p>
                                </div>
                            </div>
                        </div>
                        {/* {isMinimized && (
                        <button
                            className="bg-[#1E9A4D] font-bold text-white p-3 shadow-2xl rounded-full hover:scale-110"
                            style={{
                                boxShadow: 'rgba(0, 0, 0, 0.25) 0px 0.0625em 0.0625em, rgba(0, 0, 0, 0.25) 0px 0.125em 0.5em, rgba(255, 255, 255, 0.1) 0px 0px 0px 1px inset'
                            }}
                            onClick={() => setIsMinimized(false)}
                        >
                            <IoInformationCircle className="md:hidden lg:block" size={40} />
                            <IoInformationCircle className="md:block lg:hidden" size={90} />
                        </button>
                    )} */}
                    </div>
                )}
